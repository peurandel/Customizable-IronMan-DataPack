...
#비행
$execute as @s[tag=!prd.ironman.fly] if items entity @s armor.chest *[minecraft:custom_data~{suit:61}] if data storage prd:db player.$(dummy).data.minecraft:custom_data.data.tech[{id:1}] unless predicate mk61:onground if predicate mk61:sneak run function mk61:player/suit/fly/tag
