...
say hi
$execute store result score #prd.this.cost prd.ironman.energy run data modify storage prd:this item.cost_e set from storage prd:db player.$(dummy).data.minecraft:custom_data.data.suit.armorcost[1][0]
$execute store result score #prd.this.cost prd.ironman.nano run data modify storage prd:this item.cost_n set from storage prd:db player.$(dummy).data.minecraft:custom_data.data.suit.armorcost[1][1]
