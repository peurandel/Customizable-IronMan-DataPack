...
$execute store result score #prd.this.cost prd.ironman.energy run data modify storage prd:this item.cost_e set from storage prd:db player.$(dummy).data.minecraft:custom_data.data.suit.armorcost[3][0]
$execute store result score #prd.this.cost prd.ironman.nano run data modify storage prd:this item.cost_n set from storage prd:db player.$(dummy).data.minecraft:custom_data.data.suit.armorcost[3][1]
...
$data modify entity @e[tag=prd.wear,limit=1] item set from storage prd:db player.$(dummy).data.minecraft:custom_data.data.suit.helmet
...
item replace entity @s armor.head from entity @e[tag=prd.wear,limit=1] container.0
kill @e[tag=prd.wear]
